...
namespace Org.RyanLiu.SmallTools
{
	...
	/// </summary>
	public class GetEmailForm : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.btnGetEmail.Size = new System.Drawing.Size(176, 23);
			this.btnGetEmail.TabIndex = 2;
			this.btnGetEmail.Text = "Get Email from Outlook express";
		...
		private String GetEmail(String text) 
		{
			...
			while( (startPos = text.IndexOf("<", endPos)+1) != 0 ) 
			{
				if( (endPos = text.IndexOf (">", startPos ) ) !=-1 )  
				{
					...
					{
						sb = sb.Append (email).Append (StringDateUtil.LINE_SPLITTER );
					}
					test = sb.ToString ();
				}
		...
		{
			tbxEmail.Text = StringDateUtil.SortLines(tbxEmail.Lines );
		}
		...
		{
			tbxEmail.Text = StringDateUtil.AddEndSymbol(tbxEmail.Lines,",");
		}
